...
const VEHICLE_IMG = "https://d2xsxph8kpxj0f.cloudfront.net/310519663471903126/n2wVeYQgKmefAVwyN2KSSe/v2_tar_sap-Lk69k55uWFLfSw8KqFBwbm.png";
...
const STEPS: Step[] = [
  {
    id: "step1",
    title: "Step 1 — Identify: Tar vs. Tree Sap",
    instruction: "Before treating, identify what you're dealing with. TAR is black, sticky, and usually found on lower panels from road spray. TREE SAP is clear/amber, sticky, and found anywhere on the vehicle from trees overhead.",
    area: "🔍 Tar/Sap — Identification",
    question: "Where is tar contamination most commonly found on a vehicle?",
    choices: [
      { id: "lower",  label: "Lower Panels",   image: ASSETS.allPurpose },
      { id: "roof",   label: "Roof & Hood",    image: ASSETS.degreaser },
      { id: "glass",  label: "Windows Only",   image: ASSETS.greenBrush },
    ],
    correctId: "lower",
    wrongExplanation: "❌ Wrong! Tar comes from road spray and is most commonly found on lower panels, rocker panels, and wheel wells.",
    correctExplanation: "✅ Correct! Tar is road spray contamination — it's most commonly found on lower body panels, rockers, and wheel wells.",
    proTip: "💡 Pro Tip: Tree sap can be anywhere — check the roof, hood, and trunk lid if the vehicle was parked under trees. It's clear/amber colored.",
  },
  {
    id: "step2",
    title: "Step 2 — Rinse the Tar Area",
    instruction: "For TAR removal: Start by rinsing the affected area with the pressure washer. This removes any loose contamination and wets the surface before applying solvent.",
    area: "🔍 Tar Removal — Pre-Rinse",
    question: "What is the first step specifically for TAR removal?",
    choices: [
      { id: "rinse",    label: "Rinse with Pressure Washer", image: ASSETS.allPurpose },
      { id: "solvent",  label: "Apply Solvent First",        image: ASSETS.degreaser },
      { id: "blade",    label: "Use Plastic Blade First",    image: ASSETS.steelWool },
    ],
    correctId: "rinse",
    wrongExplanation: "❌ Wrong! Always rinse first before applying solvent. Applying solvent to a dirty surface reduces its effectiveness.",
    correctExplanation: "✅ Correct! Rinse the tar area first with the pressure washer to remove loose contamination before applying solvent.",
    proTip: "💡 Pro Tip: Clean around the tar area with a damp All-Purpose Towel before applying solvent — this prevents spreading contamination.",
  },
  {
    id: "step3",
    title: "Step 3 — Apply Solvent Remover for Tar",
    instruction: "Apply solvent remover to an All-Purpose Towel and gently rub the tar area. The solvent dissolves the tar so it wipes away cleanly. If the tar is hard to remove, use a plastic razor blade to help agitate.",
    area: "🔍 Tar Removal — Solvent",
    question: "If tar is hard to remove with solvent alone, what tool can you use?",
    choices: [
      { id: "blade",    label: "Plastic Razor Blade", image: ASSETS.steelWool },
      { id: "steelWool","label": "Steel Wool",        image: ASSETS.detailBrush },
      { id: "greenBrush","label": "Green Brush",      image: ASSETS.greenBrush },
    ],
    correctId: "blade",
    wrongExplanation: "❌ Wrong! Steel wool and green brush will scratch the paint. Use a PLASTIC razor blade — it's safe for paint surfaces.",
    correctExplanation: "✅ Correct! A plastic razor blade can help agitate stubborn tar without scratching the paint surface.",
    proTip: "💡 Pro Tip: Hold the plastic blade at a very shallow angle (10-15°) to the paint surface. Never use a metal blade on paint.",
  },
  {
    id: "step4",
    title: "Step 4 — Tree Sap Removal",
    instruction: "For TREE SAP: Apply Sap Remover to an All-Purpose Towel and agitate the tree sap. Let the product sit for 30 seconds before wiping — this gives it time to break down the sap.",
    area: "🌳 Tree Sap Removal",
    question: "How long should you let Sap Remover sit before wiping?",
    choices: [
      { id: "30sec",  label: "30 Seconds",  image: ASSETS.allPurpose },
      { id: "5min",   label: "5 Minutes",   image: ASSETS.degreaser },
      { id: "nowipe", label: "Wipe Immediately", image: ASSETS.greenBrush },
    ],
    correctId: "30sec",
    wrongExplanation: "❌ Wrong! Wiping immediately doesn't give the product time to break down the sap. 5 minutes is too long — the product may dry.",
    correctExplanation: "✅ Correct! Let Sap Remover sit for 30 seconds — this gives it time to penetrate and break down the tree sap before wiping.",
    proTip: "💡 Pro Tip: For hardened sap, apply the sap remover and cover with plastic wrap for 30 seconds — this keeps the product wet and increases dwell time.",
  },
];
...
export default function TarSapRemovalTraining() {
  ...
  const { applyOverrides, moduleVideoUrl: hookModuleVideoUrl } = useStepOverrides("tar-sap-removal");
  ...
  const handleNext = useCallback(async () => {
    if (currentStep < STEPS.length - 1) {
      ...
      if (employee?.employeeId) {
        await progressMutation.mutateAsync({ employeeId: employee.employeeId, moduleId: "TM_TARSAP", completedSteps: steps.map((s) => s.id).join(","), isModuleCompleted: "yes", completedAt: new Date() });
      }
    }
    ...
    return (
      <ScreenContainer edges={["left", "right"]} className="p-6">
        <ScrollView contentContainerStyle={{ flexGrow: 1, alignItems: "center", justifyContent: "center", gap: 24, paddingVertical: 32 }}>
          ...
          <Text style={{ fontSize: 28, fontWeight: "800", color: colors.foreground, textAlign: "center" }}>{passed ? "Tar/Sap Certified!" : "Keep Practicing!"}</Text>
          <Text style={{ fontSize: 16, color: colors.muted, textAlign: "center" }}>{passed ? "Tar & Sap Removal module complete. Progress saved." : "Review the steps and try again."}</Text>
    ...
    return (
      <ScreenContainer edges={["left", "right"]}>
        <InteractiveVideoGate
          ...
          watchKey={"tar-sap-removal_intro"}
          title={"Tar Sap Removal"}
  ...
  if (moduleVideoWatched && !toolsViewed) {
    return (
      <ScreenContainer edges={["left", "right"]}>
        <ModuleToolsScreen
          moduleKey={"tar-sap-removal"}
          moduleName={"Tar Sap Removal"}
    ...
    <ScreenContainer edges={["left", "right"]}>
      <ScrollView ref={scrollRef} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
        <View style={{ paddingHorizontal: 16, paddingTop: Math.max(insets.top, 12), paddingBottom: 8, flexDirection: "row", alignItems: "center", gap: 12 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 13, fontWeight: "600", color: colors.muted }}>🔍 Tar / Sap Removal</Text>

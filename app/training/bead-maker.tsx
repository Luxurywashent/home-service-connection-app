...
const VEHICLE_IMG = "https://d2xsxph8kpxj0f.cloudfront.net/310519663471903126/n2wVeYQgKmefAVwyN2KSSe/v2_bead_maker-FXCNspASdLMM7Gfe3h9roz.png";
...
const STEPS: Step[] = [
  {
    id: "step1",
    title: "Step 1 — Apply Bead Maker to Wet Paint",
    instruction: "After drying, apply Bead Maker to the entire painted surface. Spray it directly onto the paint — you can apply it to a wet or dry surface, but application on a slightly damp surface gives the best results.",
    area: "✨ Bead Maker — Application",
    question: "When can you apply Bead Maker to the vehicle?",
    choices: [
      { id: "wetDry",  label: "Wet OR Dry Surface", image: ASSETS.allPurpose },
      { id: "dryOnly", label: "Dry Surface Only",   image: ASSETS.degreaser },
      { id: "wetOnly", label: "Wet Surface Only",   image: ASSETS.washMitt },
    ],
    correctId: "wetDry",
    wrongExplanation: "❌ Wrong! Bead Maker is versatile — it can be applied to both wet and dry surfaces. This makes it very easy to use.",
    correctExplanation: "✅ Correct! Bead Maker can be applied to wet OR dry surfaces — this is one of its key advantages over traditional waxes.",
    proTip: "💡 Pro Tip: Apply Bead Maker in the shade when possible — direct sunlight causes it to dry too quickly before you can spread it evenly.",
  },
  {
    id: "step2",
    title: "Step 2 — Spread with All-Purpose Microfiber",
    instruction: "Use the All-Purpose Microfiber towel to spread the Bead Maker evenly across each panel. Work one panel at a time — hood, roof, trunk, then sides.",
    area: "✨ Bead Maker — Spreading",
    question: "Which towel do you use to spread Bead Maker?",
    choices: [
      { id: "allPurpose", label: "All-Purpose Microfiber", image: ASSETS.allPurpose },
    ...
    wrongExplanation: "❌ Wrong! Never use a brush or wash mitt to apply Bead Maker. The All-Purpose Microfiber towel is the correct tool.",
    correctExplanation: "✅ Correct! The All-Purpose Microfiber towel is used to spread Bead Maker — its soft fibers distribute the product evenly.",
    proTip: "💡 Pro Tip: Use light, overlapping strokes when spreading Bead Maker — this ensures even coverage without leaving thick spots.",
  },
  {
    id: "step3",
    title: "Step 3 — Buff to a Shine",
    instruction: "After spreading, use a clean, dry All-Purpose Microfiber towel to buff the Bead Maker to a shine. Use circular motions for the final buff. The paint should feel slick and look glossy.",
    area: "✨ Bead Maker — Buffing",
    question: "What motion do you use when buffing Bead Maker to a shine?",
    choices: [
      { id: "circular",  label: "Circular Motions",  image: ASSETS.allPurpose },
      { id: "straight",  label: "Straight Lines Only", image: ASSETS.degreaser },
      { id: "noMotion",  label: "Just Wipe Once",     image: ASSETS.greenBrush },
    ],
    correctId: "circular",
    wrongExplanation: "❌ Wrong! For the final buff, use circular motions — this distributes the product evenly and creates a uniform shine.",
    correctExplanation: "✅ Correct! Use circular motions for the final buff — this evenly distributes the Bead Maker and creates a deep, uniform shine.",
    proTip: "💡 Pro Tip: Use a separate clean towel for buffing — the towel used to spread the product is already saturated and won't buff effectively.",
  },
  {
    id: "step4",
    title: "Step 4 — Apply to ALL Painted Surfaces",
    instruction: "Apply Bead Maker to ALL painted surfaces — hood, roof, trunk, all four doors, front and rear bumpers, and fenders. Don't skip any panels.",
    area: "✨ Bead Maker — Coverage",
    question: "Which surfaces do you apply Bead Maker to?",
    choices: [
      { id: "all",    label: "All Painted Surfaces", image: ASSETS.allPurpose },
      { id: "top",    label: "Top Panels Only",      image: ASSETS.degreaser },
      { id: "hood",   label: "Hood & Trunk Only",    image: ASSETS.washMitt },
    ],
    correctId: "all",
    wrongExplanation: "❌ Wrong! Bead Maker must be applied to ALL painted surfaces — skipping panels creates uneven protection and appearance.",
    correctExplanation: "✅ Correct! Apply Bead Maker to ALL painted surfaces — every panel needs protection for a consistent finish.",
    proTip: "💡 Pro Tip: Don't forget the door jambs — apply a light coat of Bead Maker to the painted surfaces inside the door jambs too.",
  },
];
...
export default function BeadMakerTraining() {
  ...
  const progressMutation = trpc.training.updateProgress.useMutation();
    ...
    { moduleId: "TM_BEADMAKER" },
    { staleTime: 5 * 60 * 1000, enabled: showQuiz }
  );
  const quizQuestions = quizQuery.data || [];
  const { applyOverrides, moduleVideoUrl: hookModuleVideoUrl } = useStepOverrides("bead-maker");
  ...
  const handleNext = useCallback(async () => {
    if (currentStep < STEPS.length - 1) {
      ...
      if (employee?.employeeId) {
        await progressMutation.mutateAsync({ employeeId: employee.employeeId, moduleId: "TM_BEADMAKER", completedSteps: steps.map((s) => s.id).join(","), isModuleCompleted: "yes", completedAt: new Date() });
      }
    }
    ...
    return (
      <ScreenContainer edges={["left", "right"]} className="p-6">
        <ScrollView contentContainerStyle={{ flexGrow: 1, alignItems: "center", justifyContent: "center", gap: 24, paddingVertical: 32 }}>
          ...
          <Text style={{ fontSize: 28, fontWeight: "800", color: colors.foreground, textAlign: "center" }}>{passed ? "Bead Maker Certified!" : "Keep Practicing!"}</Text>
          <Text style={{ fontSize: 16, color: colors.muted, textAlign: "center" }}>{passed ? "Bead Maker module complete. Progress saved." : "Review the steps and try again."}</Text>
  ...
  if (moduleVideoWatched && !toolsViewed) {
    return (
      <ScreenContainer edges={["left", "right"]}>
        <ModuleToolsScreen
          moduleKey={"bead-maker"}
          moduleName={"Bead Maker"}
    ...
    <ScreenContainer edges={["left", "right"]}>
      <ScrollView ref={scrollRef} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
        <View style={{ paddingHorizontal: 16, paddingTop: Math.max(insets.top, 12), paddingBottom: 8, flexDirection: "row", alignItems: "center", gap: 12 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 13, fontWeight: "600", color: colors.muted }}>✨ Bead Maker</Text>

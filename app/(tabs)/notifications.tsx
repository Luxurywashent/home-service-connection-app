...
import { Text, View, FlatList, TouchableOpacity, ActivityIndicator, Modal, ScrollView, Linking, StyleSheet } from "react-native";
...
export default function AlertsScreen() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "writeups" | "meetings">("all");
  ...
  const { data: upcomingMeetings, isLoading: meetingsLoading } = trpc.companyMeetings.getUpcoming.useQuery();

  const formatMeetingDate = (dateStr: string, timeStr: string) => {
    try {
      const d = new Date(`${dateStr}T${timeStr}:00`);
      return d.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" }) + " at " + d.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true });
  ...
  return (
    <ScreenContainer edges={["left", "right"]} className="px-5">
      ...
      <View style={{ marginBottom: 14 }}>
        <View style={{ flexDirection: "row", backgroundColor: colors.surface, borderRadius: 10, padding: 3, borderWidth: 1, borderColor: colors.border, gap: 2, overflow: "hidden" }}>
          <TouchableOpacity
            onPress={() => setActiveTab("all")}
            style={{ height: 36, paddingHorizontal: 14, borderRadius: 8, alignItems: "center", justifyContent: "center", backgroundColor: activeTab === "all" ? colors.primary : "transparent", overflow: "hidden" }}
          >
            <Text style={{ color: activeTab === "all" ? "#fff" : colors.muted, fontSize: 13, fontWeight: "700", lineHeight: 16 }}>
              All Alerts {unreadCount > 0 ? `(${unreadCount})` : ""}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setActiveTab("writeups")}
            style={{ height: 36, paddingHorizontal: 14, borderRadius: 8, alignItems: "center", justifyContent: "center", backgroundColor: activeTab === "writeups" ? colors.error : "transparent", overflow: "hidden" }}
          >
            <Text style={{ color: activeTab === "writeups" ? "#fff" : colors.muted, fontSize: 13, fontWeight: "700", lineHeight: 16 }}>
              Write-Ups {unreadWriteUps > 0 ? `(${unreadWriteUps})` : writeUps.length > 0 ? `(${writeUps.length})` : ""}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setActiveTab("meetings")}
            style={{ height: 36, paddingHorizontal: 14, borderRadius: 8, alignItems: "center", justifyContent: "center", backgroundColor: activeTab === "meetings" ? "#7C3AED" : "transparent", overflow: "hidden" }}
          >
            <Text style={{ color: activeTab === "meetings" ? "#fff" : colors.muted, fontSize: 13, fontWeight: "700", lineHeight: 16 }}>
              Meetings {(upcomingMeetings?.length ?? 0) > 0 ? `(${upcomingMeetings!.length})` : ""}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
          ...
          initialNumToRender={10}
            ...
            ListEmptyComponent={
              <View style={{ alignItems: "center", marginTop: 60 }}>
                ...
                <Text style={{ fontSize: 16, color: colors.muted }}>No upcoming meetings</Text>
                <Text style={{ fontSize: 13, color: colors.muted, marginTop: 6, textAlign: "center" }}>Check back later for scheduled company meetings</Text>
              </View>
            }
            renderItem={({ item }: { item: any }) => (
              <View style={{
                backgroundColor: colors.surface, borderRadius: 14, padding: 16, marginBottom: 10,
                borderWidth: 1, borderColor: "#7C3AED40", borderLeftWidth: 4, borderLeftColor: "#7C3AED",
              }}>
                ...
                <Text style={{ color: "#7C3AED", fontSize: 13, marginTop: 4, fontWeight: "600" }}>{formatMeetingDate(item.meetingDate, item.meetingTime)}</Text>
                {item.isRecurring === "yes" && item.recurringDay && (
                  <Text style={{ color: colors.muted, fontSize: 12, marginTop: 2 }}>Every {item.recurringDay}</Text>
                )}
                {item.description ? (
                  <Text style={{ color: colors.muted, fontSize: 13, marginTop: 8, lineHeight: 20 }}>{item.description}</Text>
                ) : null}
                {item.zoomLink ? (
                  <TouchableOpacity
                    onPress={() => Linking.openURL(item.zoomLink)}
                    activeOpacity={0.7}
                    style={{ marginTop: 12, backgroundColor: "#2D8CFF", borderRadius: 10, paddingVertical: 12, alignItems: "center" }}
                  >
                    <Text style={{ color: "#FFF", fontSize: 14, fontWeight: "700" }}>🎥 Join Zoom Meeting</Text>
                  </TouchableOpacity>
                ) : null}
              </View>
            )}

...
function LegacySalesTimesheetScreen() {
  ...
  const handleEditTime = (recordId: string, currentValue: string) => {
  ...
  const handleSaveTime = async (recordId: string, isClockIn: boolean) => {
    ...
    try {
      ...
      const [time, period] = editingValue.split(" ");
      ...
      const newDateTime = new Date(`${dateStr}T${String(hour).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:00`);
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: 12, color: colors.muted }}>
                Submit a time-off request for approval (contact admin)
              </Text>
            </View>
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          {editingId === `${log.recordId}-in` ? (
                            <View style={{ flexDirection: "row", gap: 8 }}>
                              ...
                              />
                              <TouchableOpacity
                                onPress={() => handleSaveTime(`${log.recordId}-in`, true)}
                                style={{ backgroundColor: colors.success, borderRadius: 8, padding: 8 }}
                              >
                            ...
                            <TouchableOpacity
                              onPress={() => handleEditTime(`${log.recordId}-in`, formatTime(log.clockInTime))}
                              style={{
                        ...
                        <View style={{ flex: 1, marginLeft: 12 }}>
                          ...
                          {editingId === `${log.recordId}-out` ? (
                            <View style={{ flexDirection: "row", gap: 8 }}>
                              ...
                              />
                              <TouchableOpacity
                                onPress={() => handleSaveTime(`${log.recordId}-out`, false)}
                                style={{ backgroundColor: colors.success, borderRadius: 8, padding: 8 }}
                              >
                                <Text style={{ color: "#FFF", fontWeight: "600", fontSize: 12 }}>✓</Text>
                              </TouchableOpacity>
                            ...
                            <TouchableOpacity
                              onPress={() => handleEditTime(`${log.recordId}-out`, formatTime(log.clockOutTime))}
                              style={{
